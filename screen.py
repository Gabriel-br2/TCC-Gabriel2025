...
class Screen:
    ...
    def __init__(self, config, color, click_callback, client_id, player_color, debug=False):
        ...
        self.font = pygame.font.SysFont("Arial", 18, bold=True)  # Font for displaying text.

        self.screen = pygame.display.set_mode((self.screen_width, self.screen_height))  # Create the Pygame screen.
        pygame.display.set_caption(f"{config['screen']['caption']} - player: {client_id} - {player_color}")  # Set window title.
    ...
    def screenshot_base64(self, save=False, file_path='screenshot/'):
        ...
        if save:  # If saving is enabled.
            ...
            dest_png = os.path.join(target_dir, os.path.basename(f"{file_name}.png"))  # Path for PNG copy.
            shutil.copy(f"{file_name}.png", dest_png)  # Copy PNG to timestamped directory.

            dest_txt = os.path.join(target_dir, os.path.basename(f"{file_name}.txt"))  # Path for Base64 text file.
            with open(dest_txt, "w") as f:  # Open text file.
                f.write(base64_string.decode("utf-8"))  # Write Base64 data to text file.
    ...
    def game_loop(self, set_new_position, get_new_position, client_socket, objects, player_id, iou, playerType):
        ...
        self.screen.fill(self.color["background"])  # Fill the screen with the background color.
        ...
        for obj in objects["me"]:
            ...
            updated_positions.append([x, y, obj.body.angle, obj.type])  # Store position, angle, and type
            obj.draw()  # Draw the objects

        new_position = updated_positions  # The updated positions to send
        set_new_position(client_socket, new_position)  # Send new position to the server.

        data = get_new_position(client_socket)  # Receive updated data from the server.
        for player_index in range(self.config["game"]["playerNum"]):  # Update other players positions
            if player_id != player_index:
                objects["you"][f"P{player_index}"]["pos"] = data[f"P{player_index}"]["pos"]  # Update other players' positions.

        iou = data["IoU"]  # Get the IoU value from the server.
        iou_text = self.font.render(f"Objetivo Concluído: {iou * 100:.2f} %", True, (0, 0, 0))  # Render the IoU text.
        text_rect = iou_text.get_rect(center=(self.config["screen"]["width"] // 2, 25))  # Create text rectangle.

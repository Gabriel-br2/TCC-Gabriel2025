...
object_options = [f.split(".")[0] for f in os.listdir("objects") if os.path.isfile(os.path.join("objects", f))]
objChoice = random.choices(object_options, k=num_objects)

modelsClass = {"generic": GenericShape(cfg).vertices, 
...
for i in range(num_players):
    ...
    for o in range(num_objects):                   
        for attempt in range(100):
            ...
            current_transformed_vertices = apply_transformation(x, y, rz, modelsClass[objChoice[o]])
    ...
    objects[f"P{i}"] = {"id": i, "color": player_colors[i], "pos": [data[:4] for data in object_positions]}
    ...
    try:
        ...
        while True:
            try:
                ...
                c = calculate_progress(cfg.data["game"]["playerNum"], goal_area, calculate_union_area(data_reorganized))
                objects["IoU"] = c

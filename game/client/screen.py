...
class Screen:
    def __init__(self, config, color, player_type, name=None, memory_path=None):
        ...
        self._pause_spinner = LoadingSpinner(
            center=(self.width // 2, self.height // 2 + 70),
            radius=max(24, min(self.width, self.height) // 28),
        )
    ...
    def game_loop(self, game_state: GameState):
        self._game_paused = game_state.is_paused
        ...
        self._render(game_state.objects, game_state.iou)
    ...
    def _handle_events(self, objects, local_objects):
        ...
        def llm_events():
            ...
            self.llm.llm_interaction(
                ...
                local_objects,
                self.iou,
                is_active=lambda: not self._game_paused,
            )
            self.lock = True
        ...
        if self.player_type == "LLM" and not self._game_paused:
            if self.lock:
                self.lock = False
    ...
    def _draw_pause_screen(self, connected_players: int, total_players: int):
        pause_font = pygame.font.SysFont("Arial", self.width // 35, bold=True)
        pause_text = pause_font.render(
            f"Jogo pausado. Aguardando jogadores {connected_players}/{total_players}.",
            True,
            (0, 0, 0),
        )
        ...
        self._pause_spinner.draw(self.screen)
        pygame.display.flip()
